-- Migration: Add phone number fields to CleverTap sync
-- Created: 2025-12-26
-- Purpose: Include phone_number and msg91_phone_number in CleverTap user sync
...
BEGIN
  ...
  -- Get user name from first profile (separate query to ensure all users with profiles get name)
  SELECT name
  INTO v_user_name
  ...
  -- Build result JSON with improved NULL handling and PHONE NUMBERS
  -- Split into multiple parts to avoid PostgreSQL's 100 argument limit for jsonb_build_object
  result := jsonb_build_object(
    -- User Identity & Phone Numbers
    'user_id', v_user.id,
    'email', v_user.email,
    'name', v_user_name,
    'phone_number', v_user.phone_number,
    'msg91_phone_number', v_user.msg91_phone_number,

    -- WhatsApp Opt-In
    'whatsapp_marketing_opt_in', COALESCE(v_user.whatsapp_marketing_opt_in, false),
    'whatsapp_opt_in_date', v_user.whatsapp_marketing_opt_in_date,
    'whatsapp_opt_in_source', v_user.whatsapp_marketing_opt_in_source,
...
-- Add comment
COMMENT ON FUNCTION public.get_user_clevertap_data(UUID) IS 'Aggregates all user data for CleverTap profile sync including name, phone numbers, sub-categories, with improved NULL handling and fixed 100-argument limit';

-- Migration: Add name field to CleverTap function
-- Created: 2025-12-26
-- Purpose: Update get_user_clevertap_data() to include user's profile name
...
BEGIN
  ...
  -- Build result JSON with improved NULL handling and PHONE NUMBERS + NAME
...
-- Add comment
COMMENT ON FUNCTION public.get_user_clevertap_data(UUID) IS 'Aggregates all user data for CleverTap profile sync including name, phone numbers, sub-categories, with improved NULL handling. Fixed to avoid 100 argument limit by splitting JSONB construction into 3 parts.';

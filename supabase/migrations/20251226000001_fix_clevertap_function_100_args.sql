-- Migration: Fix CleverTap function to avoid 100 argument limit
-- Created: 2025-12-26
-- Purpose: Update get_user_clevertap_data() to split JSONB construction into 3 parts
...
BEGIN
  ...
  -- Get birth details and name from first profile (with NULL handling)
  SELECT ubd.gender, ubd.date_of_birth, up.name as profile_name
  ...
  result := jsonb_build_object(
    ...
    'user_id', v_user.id,
    'email', v_user.email,
    'name', v_birth_details.profile_name,
...
-- Add comment
COMMENT ON FUNCTION public.get_user_clevertap_data(UUID) IS 'Aggregates all user data for CleverTap profile sync including phone numbers, sub-categories, with improved NULL handling. Fixed to avoid 100 argument limit by splitting JSONB construction into 3 parts.';

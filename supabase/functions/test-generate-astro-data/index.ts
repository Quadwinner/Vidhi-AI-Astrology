...
async function handler(req: Request) {
  ...
  try {
    ...
    const { profile_id, scope, report_type, monetization_variant } = await req.json();
    ...
    if (scope === 'ai') {
      ...
      if (serviceKey === 'report_premium' && isPremium) {
        ...
        // We now pass the monetization_variant to the helper function.
        const deduction = await processWalletDeduction(
          supabaseAdmin,
          user.id,
          serviceKey,
          1,
          monetization_variant || 'control' // Pass the variant, defaulting to 'control' for safety
        );
        ...
        console.log(`[Report] Unlocked ${report_type}. Cost: ${deduction.deducted} ${deduction.currency} (Variant: ${monetization_variant || 'control'})`);
      }
...
async function generateAiInsights(
  ...
  } else if (apiProvider === 'google' || apiProvider === 'gemini') {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${apiKey}`;
    const body = { contents: [{ role: "user", parts: [{ text: finalUserPrompt }] }], systemInstruction: { parts: [{ text: systemPrompt }] }, generationConfig: { temperature: 0.2 } };
    const resp = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!resp.ok) { const errText = await resp.text(); throw new Error(`Gemini API error (${resp.status}): ${errText.slice(0, 300)}`); }
    const result = await resp.json();
    analyst_report = result?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || null;
  } else if (apiProvider === 'openrouter') {
    const baseUrl = modelName.startsWith('accounts/fireworks/') ? "https://api.fireworks.ai/inference/v1/chat/completions" : "https://openrouter.ai/api/v1/chat/completions";
    const resp = await fetch(baseUrl, { method: "POST", headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: modelName, temperature: 0.2, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: finalUserPrompt }] }) });
    if (!resp.ok) { const errText = await resp.text(); throw new Error(`API error (${resp.status}): ${errText.slice(0, 300)}`); }
    const data = await resp.json();
    analyst_report = data?.choices?.[0]?.message?.content?.trim() || null;
...
async function processWalletDeduction(
  ...
  serviceKey: string,
  quantity: number = 1,
  variantName: string = 'control' // <-- Accepts the variant from the handler
) {
  // 1. Get User's Currency & Balance (no change)
  ...
  // --- 2. THE CRITICAL QUERY UPDATE ---
  // Get Price for this Service + Currency + VARIANT
  ...
  const { data: priceData, error: priceError } = await supabaseAdmin
    ...
    .eq('variant_name', variantName) // <-- We now filter by the user's variant!
    .single();

  if (priceError || !priceData) {
    // This error message is now more informative for debugging
